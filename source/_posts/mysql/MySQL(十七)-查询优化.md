...
* InnoDB引擎会遍历整张表,把每一行号的ID值都取出来,返回给Server层.Server层拿到Id后,判断是不可能为空的,就按行累加.
...
* 如果这个"字段"定义允许为NULL的话,那么执行的时候,判断到有可能是NULL,还要把值取出来再判断一下,不是NULL才累加.

##### count(*)

* count(*)是列外,并不会 把全部字段取出来,而是专门做了优化,不取值.count(\*)肯定不是NULL,按行累加. 
